 ...
 */
public class DiagnosisAdapter extends ArrayAdapter<String> {

    Context context;
    ArrayList<Diagnosis> data;

    public DiagnosisAdapter(Context context, ArrayList<Diagnosis> data) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        fullname.setText(data.get(position).getUser().getFullname());
        mobile.setText(data.get(position).getUser().getMobile());
        date.setText(new Date(data.get(position).getTime()).getTimeAgo());
        new Image(imageView, data.get(position).getUser().getImage());
